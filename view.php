...
if (mysqli_num_rows($result) > 0) {
    ...
    while($row = mysqli_fetch_assoc($result)) {
		...
		//$ss = "SELECT * FROM budget where cid='$cid'";
		//$tt = mysqli_query($connect, $ss);
		...
		while($row = mysqli_fetch_assoc($resultt)) {
			echo "<td> " ."<FONT COLOR='black' size='4'>". $row["hname"]. $h."</td>";
		}
		while($row = mysqli_fetch_assoc($resul)) {
			echo "<td>" . "<FONT COLOR='black' size='4'>".$row["tname"].$t."</td>";
		}
		while($row = mysqli_fetch_assoc($x)) {
			echo "<td> " ."<FONT COLOR='black' size='4'>". $row["mname"].$m."</td>";
		}
		while($row = mysqli_fetch_assoc($y)) {
			echo "<td>" ."<FONT COLOR='black' size='4'>". $row["vname"].$v."</td>";
		}
		while($row = mysqli_fetch_assoc($z)) {
			echo "<td> " ."<FONT COLOR='black' size='4'>". $row["pname"].$p."</td>";
		}//while($row = mysqli_fetch_assoc($tt)) {
			echo "<td>" . "<FONT COLOR='black' size='4'>".$tt."</td>";
		//}
		echo"</tr>";
